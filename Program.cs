...
namespace F1_Season_2023_Ew
{
    public class Data
    {
        public static readonly string UserDataPath = Path.Combine(Environment.CurrentDirectory, "UserData");

        public bool ChangeUserData = false;
        public static int Difficulty { get; set; }
        public static void LoadDifficulty()
        {
            string path = Path.Combine(UserDataPath, "Difficulty.txt");
            StreamReader difficultyR = new(path);
        ...
        {
            string path = Path.Combine(UserDataPath, "Difficulty.txt");
            StreamWriter difficultyW = new(path);
        ...
        {     //Team            Full Name         SrtNm  Num  Country          Race Data    AG     AF    FL DNF
            new(TeamList()[0], "Lewis Hamilton",  "HAM", 44, "United Kingdom", Tuple.Create( 5.88,  4.12, 3, 1)),
            new(TeamList()[0], "George Russell",  "RUS", 63, "United Kingdom", Tuple.Create( 6.90,  5.38, 2, 3)),
            new(TeamList()[1], "Max Verstappen",  "VER",  1, "Netherlands",    Tuple.Create( 3.30,  1.20, 7, 1)),
            new(TeamList()[1], "Sergio Perez",    "PER", 11, "Mexico",         Tuple.Create( 7.75,  4.60, 3, 2)),
            new(TeamList()[2], "Charles Leclerc", "LEC", 16, "Monaco",         Tuple.Create( 7.12,  6.13, 2, 3)),
            new(TeamList()[2], "Carlos Sainz",    "SAI", 55, "Spain",          Tuple.Create( 4.62,  6.25, 3, 2)),
            new(TeamList()[3], "Lando Norris",    "NOR",  4, "United Kingdom", Tuple.Create( 9.20, 11.10, 1, 1)),
            new(TeamList()[3], "Oscar Piastri",   "PIA", 81, "Australia",      Tuple.Create(11.50, 11.89, 0, 2)),
            new(TeamList()[4], "Esteban Ocon",    "OCO", 31, "France",         Tuple.Create( 9.30,  9.80, 1, 4)),
            new(TeamList()[4], "Pierre Gasly",    "GAS", 10, "France",         Tuple.Create(11.10, 11.00, 1, 3)),
            new(TeamList()[5], "Yuki Tsunoda",    "TSU", 22, "Japan",          Tuple.Create(13.75, 11.50, 1, 1)),
            new(TeamList()[5], "Daniel Ricciardo","RIC",  3, "Australia",      Tuple.Create(11.00, 10.00, 1, 1)),
            new(TeamList()[6], "Lance Stroll",    "STR", 18, "Canada",         Tuple.Create( 9.90,  8.38, 1, 4)), 
            new(TeamList()[6], "Fernando Alonso", "ALO", 14, "Spain",          Tuple.Create( 4.70,  3.90, 1, 1)),
            new(TeamList()[7], "Alexander Albon", "ALB", 23, "Thailand",       Tuple.Create(12.10, 11.50, 0, 3)),
            new(TeamList()[7], "Logan Sargeant",  "SAR",  2, "United States",  Tuple.Create(17.40, 15.78, 0, 3)),
            new(TeamList()[8], "Valtteri Bottas", "BOT", 77, "Finland",        Tuple.Create(14.70, 13.50, 0, 1)),
            new(TeamList()[8], "Zhou Guanyu",     "ZHO", 24, "China",          Tuple.Create(15.60, 13.22, 2, 2)),
            new(TeamList()[9], "Kevin Magnussen", "MAG", 20, "Denmark",        Tuple.Create(14.90, 15.00, 0, 4)),
            new(TeamList()[9], "Nico Hulkenberg", "HUL", 27, "Germany",        Tuple.Create(11.10, 14.00, 0, 2))
        };
        ...
        public static int DriversFastestLapStatSum()
        {
            int sum = 0;
            foreach (var driver in DriverData())
                sum += driver.Item6.Item3 * 5 + 5;
            return sum;
        }
        public List<Tuple<string?, string?, string?, string?, string?>> UserData()
        {
            ...
            string? firstName, lastName, fullName, shortName, nationality, firstNameRead = "", 
                saveFilePath = Path.Combine(UserDataPath, "SaveFile.txt"), blankSpace = "";
            try
            {
                StreamReader saveFileR = new(saveFilePath);
            ...
            { 
                Console.WriteLine($"{Colors.Text}Can't read save file\nCheck this path - " + @$"{Colors.Gold}{saveFilePath}{Colors.Text}");
            ...
            if (firstNameRead.IsNullOrEmpty() || ChangeUserData)
            {
                ...
                try
                {
                    StreamWriter saveFileW = new(saveFilePath);
        ...
        private static string ShortNameCreator(string? firstName, string? lastName)
        {
            ...
            if (lastName.Length == 3)
                shortName = lastName;
            else if (firstName.Length + lastName.Length < 3 || firstName.Length + lastName.Length == 3)
    ...
    public class Race
    {
        ...
        public static void FullRace(Data data, Random rnd, List<Tuple<string, string, string, int, string, Tuple<double, double, int, int>>> driver, int userKey, int circuit, List<Tuple<int, int>> userTeamNumber, List<KeyValuePair<int, double>> startingLineup, WeatherSet.Weather weather, string userTeamColor)
        {
            ...
            int newPos = 0, oldPos = 0, score = 0, totalLaps = Data.CircuitData()[circuit].Item7, pitCount = 0, noRepeats = 0, randomMiniGame = 0, 
                miniGameCounter = 1, winner = 0, flHolder = 0, otherDriverInOvertake, listDNFLast;
            string posIndicator;
            double rndDouble;
            ...
            for (int lap = 1; lap <= Data.CircuitData()[circuit].Item7;)
            {
                ...
                if (flag == Flag.Green && lastFlag == Flag.Green)
                {
                    ...
                    if (userDNF == false)
                    {
                        ...
                        else if (randomMiniGame == 2 && lap > 3 && newPos != 0)
                        {
                            ...
                            if (randomMiniGame == 0 && (int)weather < 1)
                                score = MiniGames.DRS();
                            else
                    ...
                    #region Check if Someone Crashed
                    ...
                    #region Update Flag 
                    if (listDNF.Count - listDNFLast > 2)
                    { 
                        flag = Flag.Red; 
                        redFlag = true; 
                    }
                    ...
                    #region Fastest Lap Update
                    driver.Sort((x, y) => y.Item6.Item3.CompareTo(x.Item6.Item3));
                    int flNumber = 0, randomFL = rnd.Next(Data.DriversFastestLapStatSum());
                ...
                {
                    Console.SetCursorPosition(0, 2);
                    Console.Write($"\r{Colors.Custom(220, 217, 2)}SAFETY CAR");
                }
                ...
                for (int i = 0; i < 20; i++)
                {
                    ...
                    for (int j = 0; j < 20; j++)
                    {
                        if (lineup[i].Key == Data.DriverData()[j].Item4)
                        {
                            ...
                            if (j == userTeamNumber[0].Item1)
                                Console.WriteLine($"{partOne}{data.UserData()[0].Item4}{fl}{Colors.Teams(j / 2)}|{Colors.Dark}{data.UserData()[0].Item3} " +
                                    $"{posIndicator + Assets.NumberToSuperscriptString(posDiff)}");
                            else
    ...
    public class EndScreen
    {
        ...
        public static void Anthem(int winner, int circuit, int userKey)
        {
            ...
            Data data = new();
            bool timeout;
            string path = Path.Combine(Environment.CurrentDirectory, "Sound"), the = "",
            ...
            try
            {
                using VorbisWaveReader anthermPath = new(@$"{path}\{driverCountry.ToLower()}.ogg"), carmenPath = new(@$"{path}\Carmen_Prelude_to_Act_1.ogg");
            ...
            {
                using VorbisWaveReader carmenPath = new(@$"{path}\Carmen_Prelude_to_Act_1.ogg");
                using WaveOutEvent play = new();
                play.Init(carmenPath);
                play.Play();
                ConsoleSemaphore.Wait();
    ...
    public class CareerMode
    {
        ...
        {
            var careerModePath = Path.Combine(Data.UserDataPath, "CareerMode.txt");
    ...
    public static class CareerManager
    {
        ...
        public static void LoadGrandPrixStats() 
        {
            try
            {
                ...
                var careerModePath = Path.Combine(Data.UserDataPath, "CareerGPs.txt");
                using var careerGPReader = new StreamReader(careerModePath);
        ...
        public static void ClearGrandPrixStats()
        {
            ...
            {
                var careerModePath = Path.Combine(Data.UserDataPath, "CareerGPs.txt");
                using var careerGPWriter = new StreamWriter(careerModePath);
