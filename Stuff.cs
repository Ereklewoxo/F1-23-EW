...
namespace F1_Season_2023_Ew
{
    public class Colors
    {
        ...
        public static string Custom(string hex)
        {
            hex = hex.TrimStart('#');
    ...
    public class Assets
    {
        public static void ASCIITextConverter(string word, int delay, string color)
        {
            ...
            foreach (char letter in word)
            {
                ...
                {   
                    if (Convert.ToInt32(letter) - 65 is >= 0 and < 26)
                    {
                        Console.Write(Colors.White + ASCIICharacters[Convert.ToInt32(letter) - 65][i]);
                        asciiLength = ASCIICharacters[Convert.ToInt32(letter) - 65][i].Length;
        ...
        {
            string path = Path.Combine(Environment.CurrentDirectory, "Sound");
            SoundPlayer player = new($@"{path}\StartingBeep.wav");
            Console.SetCursorPosition(0, line);
            Console.Write(Colors.Grayer + "\r           •••••" + Colors.Red);
